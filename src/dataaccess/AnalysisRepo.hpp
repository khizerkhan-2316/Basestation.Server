...
private:
    ...
    Analysis deserialize(const std::map<std::string, std::string>& record) {
        ...
        while (std::getline(measurementsStream, measurementData, ';')) {
            ...
            analysis.addMeasurement(std::stof(temp), std::stof(oxygen), std::stof(pressure));
        }
        ...
        // Deserialize status
        analysis.setStatus(static_cast<AnalysisStatus>(std::stoi(record.at("status"))));

        return analysis;
    }

public:
    explicit AnalysisRepository(const std::string& filePath) : dbUtils(filePath) {}
